...
class BannerViewPager<T, VH : BaseViewHolder<T>?> @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : RelativeLayout(context, attrs, defStyleAttr) {
    ...
    private val mOnPageChangeCallback: OnPageChangeCallback = object : OnPageChangeCallback() {
        override fun onPageScrolled(position: Int, positionOffset: Float, positionOffsetPixels: Int) {
            ...
            val realPosition = getRealPosition(isCanLoop, position, listSize)
            if (listSize > 0) {
        ...
        override fun onPageSelected(position: Int) {
            ...
            currentPosition = getRealPosition(isCanLoop, position, size)
    ...
    override fun onInterceptTouchEvent(ev: MotionEvent): Boolean {
        if (!mViewPager.isUserInputEnabled || adapter!!.data!!.size <= 1) {
    ...
    private fun onVerticalActionMove(endY: Int, disX: Int, disY: Int) {
        if (disY > disX) {
            if (!isCanLoop) {
                ...
                } else if (currentPosition == data!!.size - 1 && endY - startY < 0) {
                    parent.requestDisallowInterceptTouchEvent(false)
                } else {
                    parent.requestDisallowInterceptTouchEvent(true)
                }
        ...
        if (disX > disY) {
            if (!isCanLoop) {
                if (currentPosition == 0 && endX - startX > 0) {
                    parent.requestDisallowInterceptTouchEvent(false)
                } else if (currentPosition == data!!.size - 1 && endX - startX < 0) {
                    parent.requestDisallowInterceptTouchEvent(false)
                } else {
                    parent.requestDisallowInterceptTouchEvent(true)
                }
    ...
    private fun setIndicatorValues(list: List<T>) {
        val indicatorVisibility = mBannerManager.bannerOptions.indicatorVisibility
    ...
    private fun initIndicatorViewMargin() {
        ...
        if (indicatorMargin == null) {
            ...
            layoutParams.setMargins(indicatorMargin.left, indicatorMargin.top,
    ...
    private val interval: Int
        private get() = mBannerManager.bannerOptions.interval

    private val isAutoPlay: Boolean
        private get() = mBannerManager.bannerOptions.isAutoPlay

    private val isCanLoop: Boolean
        private get() = mBannerManager.bannerOptions.isCanLoop
    ...
    fun refreshData(list: List<T>?) {
        if (list != null && adapter != null) {
            ...
            if (mIndicatorView != null) {
                val indicatorOptions = mBannerManager.bannerOptions.indicatorOptions
                indicatorOptions.pageSize = list.size
                indicatorOptions.currentPosition = getRealPosition(isCanLoop, mViewPager.currentItem, list.size)
                mIndicatorView?.notifyDataChanged()
            }
            startLoop()
        }
    ...
    var currentItem: Int
        get() = currentPosition
        set(item) {
            if (isCanLoop && adapter!!.listSize > 1) {
                mViewPager.currentItem = BaseBannerAdapter.MAX_VALUE / 2 - BaseBannerAdapter.MAX_VALUE / 2 % adapter!!.listSize + 1 + item
            } else {
    ...
    fun setCurrentItem(item: Int, smoothScroll: Boolean) {
        if (isCanLoop && adapter!!.listSize > 1) {
            mViewPager.setCurrentItem(BaseBannerAdapter.MAX_VALUE / 2 - BaseBannerAdapter.MAX_VALUE / 2 % adapter!!.listSize + 1 + item, smoothScroll)
        } else {
    ...
    fun showIndicator(showIndicator: Boolean): BannerViewPager<T, VH> {
        mIndicatorLayout?.visibility = if (showIndicator) View.VISIBLE else View.GONE
        return this
    }

    @Deprecated("user {@link #setUserInputEnabled(boolean)} instead.")
    fun disableTouchScroll(disableTouchScroll: Boolean): BannerViewPager<T, VH> {
        mBannerManager.bannerOptions.isUserInputEnabled = !disableTouchScroll
        return this
    }
